...
void SchoolSkipperClient::initGraphicsViews() {
     ...
     * it leaves it as 0 therefore no views are being created; tried to fix it with a forced ceiling - didnt work
     */
...
void SchoolSkipperClient::messageReadyToSendSlot() {
    ...
    std::string withPrefix = "Message "+message.toStdString();
    emit messageReadyToSendSignal(QByteArray::fromStdString(withPrefix));
