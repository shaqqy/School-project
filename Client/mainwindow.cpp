...
//  QGraphicsPixmapItem *singleplayer = new QGraphicsPixmapItem(QPixmap(":/images/images/play_menu.png"));
//  this->graphicsScene->addItem(singleplayer);
//  singleplayer->setPos(250,-350);
//  connect(singleplayer, &QGraphicsItem::)
  initStartButtons();
//  QPushButton *singleplayer = new QPushButton(QIcon(QPixmap(":/images/images/play_menu.png")));
...
void SchoolSkipperClient::initStartButtons(){
  ...
  connect(single, &QPushButton::clicked, this, &SchoolSkipperClient::hideStartButtons);
  ...
  connect(multi, &QPushButton::clicked, this, &SchoolSkipperClient::hideStartButtons);
}
...
void SchoolSkipperClient::keyPressEvent(QKeyEvent *event) {
  ...
  case Qt::Key_Backspace:
            chatFrame->getInputBox()->setText(QString::fromStdString(
                chatFrame->getInputBox()->text().toStdString().substr(0,
                chatFrame->getInputBox()->text().length() - 1)));
    break;
  default:
    chatFrame->getInputBox()->setText(chatFrame->getInputBox()->text()+event->text());
    //    default:
